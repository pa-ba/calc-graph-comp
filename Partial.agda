...
--
-- This code is adapted from the code repository for the paper:
-- "Monadic Compiler Calculation" (Patrick Bahr and Graham Hutton,
-- ICFP 2022).
------------------------------------------------------------------------
...
return : ∀ {i A} → A → Partial A i
return = now

_>>_ : ∀ {i A B} → Partial A i → Partial B i → Partial B i
p >> q = p >>= λ _ → q
...
~i>>=-cong : ∀ {i A B}  {a b : Partial A ∞} (eq : a ~[ i ] b)
            {k l : A → Partial B ∞} (h : ∀ a → (k a) ~[ i ] (l a)) →
            (a >>= k) ~[ i ] (b >>= l)
~i>>=-cong (~izero) g = ~izero
~i>>=-cong (~inow _ a) g =  g a
~i>>=-cong {suc i} (~ilater h) g =  ~ilater ( ~i>>=-cong h \ a' -> ~idown (g a'))

~i>>=-cong-l : ∀ {i A B}  {a b : Partial A ∞} (eq : a ~[ i ] b)
            {k : A → Partial B ∞} →
            (a >>= k) ~[ i ] (b >>= k)
~i>>=-cong-l (~izero) = ~izero
~i>>=-cong-l (~inow a _) =  ~irefl
~i>>=-cong-l (~ilater eq) = ~ilater ( ~i>>=-cong-l eq)


~i>>=-cong-r : ∀ {i A B}  (a : Partial A ∞)
            {k l : A → Partial B ∞} (h : ∀ a → (k a) ~[ i ] (l a)) →
            (a >>= k) ~[ i ] (a >>= l)
~i>>=-cong-r (now x) eq =  eq x
~i>>=-cong-r {zero} (later x) eq =  ~izero
~i>>=-cong-r {suc i} (later x) eq = ~ilater (~i>>=-cong-r (force x) \ a' -> ~idown (eq a') )

~i>>=-assoc : ∀{i A B C}(m : Partial A ∞)
                 {k : A → Partial B ∞}{l : B → Partial C ∞} →
                 ((m >>= k) >>= l) ~[ i ] (m >>= λ a → k a >>= l)
~i>>=-assoc (now x) =  ~irefl
~i>>=-assoc {zero} (later x) =  ~izero
~i>>=-assoc {suc i} (later x) =  ~ilater ( ~i>>=-assoc (force x))

~i>>=-assoc' : ∀ {A B C i} (p : Partial A ∞)
                {k : A → Partial B ∞}{l : B → Partial C ∞}{f : A → Partial C ∞} →
                (∀ a → k a >>= l ~[ i ] f a) →
                ((p >>= k) >>= l) ~[ i ] (p >>= f)
...
~inever->>= : ∀ {i A B} {f : A → Partial B ∞} → (never >>= f) ~[ i ] never
~inever->>= {0} = ~izero
~inever->>= {suc i} =  ~ilater ~inever->>=

~i>>=-never : ∀ {i A B} (m : Partial A ∞) → _~[_]_ {B} (m >>= (λ x → never)) i never
~i>>=-never {zero} m = ~izero
~i>>=-never {suc i} (now x) = ~irefl
~i>>=-never {suc i} (later x) =  ~ilater ( ~i>>=-never (force x))
...
match-cong-default {i} {A} c m {d} {e} {f} h =  ~i>>=-cong-r m   cases
...
match-cong {i} {A} c m {d} {f} {g} e =  ~i>>=-cong-r m  cases
